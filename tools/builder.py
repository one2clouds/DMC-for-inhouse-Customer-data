...
def resume_model(base_model, args, logger = None):
    ...
    base_ckpt = {k.replace("basic_SingleConv", "basic_module.SingleConv"): v for k, v in base_ckpt.items()}
    ...
    if not isinstance(state_dict['best_metrics'], dict):
        best_metrics['CDL1'] = state_dict['best_metrics']
    else: 
